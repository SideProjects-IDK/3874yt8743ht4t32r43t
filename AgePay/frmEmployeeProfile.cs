...
namespace AgePay
{
    public partial class frmEmployeeProfile : Form
    {
        ...
        public frmEmployeeProfile()
        {
            ...
            SetupFocusEvents();
            txtDOB.Click += TxtDOB_Click;
            txtDOA.Click += TxtDOA_Click;
            txtDOR.Click += TxtDOR_Click; // Added for DOR calendar
        }

        private void TxtDOR_Click(object sender, EventArgs e)
        {
            using (MonthCalendar calendar = new MonthCalendar())
            {
                Form calendarForm = new Form
                {
                    Text = "Select Date of Retirement",
                ...
                if (DateTime.TryParse(txtDOR.Text, out DateTime selectedDate))
                {
                    calendar.SetDate(selectedDate);
                }
                calendar.DateSelected += (s, args) =>
                {
                    txtDOR.Text = args.Start.ToString("yyyy-MM-dd");
                    calendarForm.Close();
                    txtCNIC.Focus(); // Move focus to next field
                };
        ...
        {
            txtEmployeeName.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; txtFatherName.Focus(); } };
            txtFatherName.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; txtDesignation.Focus(); } };
            txtDesignation.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; comboBox_txtDeptID.Focus(); } };
            comboBox_txtDeptID.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; txtDOB.Focus(); } };
            txtDOB.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; txtDOA.Focus(); } };
            txtDOA.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; txtDOR.Focus(); } };
            txtDOR.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; txtCNIC.Focus(); } };
            txtCNIC.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; txtGSalary.Focus(); } };
            txtGSalary.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; txtDepoNo.Focus(); } };
            txtDepoNo.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; txtDutyIn.Focus(); } };
            txtDutyIn.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; txtDutyOut.Focus(); } };
            txtDutyOut.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) { e.SuppressKeyPress = true; btnSave.Focus(); } };
        }
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Make image selection optional
                DialogResult imagePrompt = MessageBox.Show("Would you like to select an employee photo?", "Select Photo", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (imagePrompt == DialogResult.Yes)
                {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (string.IsNullOrWhiteSpace(txtEmployeeID.Text) ||
                        ...
                        comboBox_txtDeptID.SelectedItem == null)
                    {
                        MessageBox.Show("Employee ID, Name, and Department are required.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                    ...
                    int deptId = (int)((dynamic)comboBox_txtDeptID.SelectedItem).DeptID;
                    ...
                    {
                        if (!DateTime.TryParseExact(txtDOR.Text, "yyyy-MM-dd", null, System.Globalization.DateTimeStyles.None, out DateTime parsedDOR))
                        {
                    ...
                    object employeeImage = DBNull.Value;
                    if (pictureEmployee.Image != null)
                    {
                        employeeImage = ImageToByteArray(pictureEmployee.Image);
                    }
                    else if (count > 0)
                    {
                        string imageQuery = "SELECT EmployeeImage FROM EmployeeProfile WHERE EmployeeID = @EmployeeID";
                        using (SqlCommand imageCommand = new SqlCommand(imageQuery, connection))
                        {
                            imageCommand.Parameters.AddWithValue("@EmployeeID", employeeId);
                            object existingImage = imageCommand.ExecuteScalar();
                            if (existingImage != DBNull.Value)
                            {
                                employeeImage = existingImage;
                            }
                        }
                    }
                    ...
                    if (count == 0)
                    {
                        if (employeeImage == DBNull.Value)
                        {
                            query = @"
                        INSERT INTO EmployeeProfile (
                            EmployeeID, EmployeeName, FatherName, Designation, DeptID, 
                            DOB, DOA, CNIC, GrossSalary, Address, DutyIn, DutyOut, UserID, PostOn
                        )
                        VALUES (
                            @EmployeeID, @EmployeeName, @FatherName, @Designation, @DeptID, 
                            @DOB, @DOA, @CNIC, @GrossSalary, @Address, @DutyIn, @DutyOut, @UserID, @PostOn
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        if (employeeImage != DBNull.Value)
                        {
                            command.Parameters.Add("@EmployeeImage", SqlDbType.VarBinary).Value = (byte[])employeeImage;
                        }
        ...
        {
            LoadDepartments();
            txtEmployeeName.Focus(); // Set initial focus
        }
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                comboBox_txtDeptID.Items.Add(new { DeptID = reader.GetInt32(0), DeptName = reader.GetString(1) });
                            }
                            comboBox_txtDeptID.DisplayMember = "DeptName";
                            comboBox_txtDeptID.ValueMember = "DeptID";
                            if (comboBox_txtDeptID.Items.Count > 0)
                            {
                                comboBox_txtDeptID.SelectedIndex = 0;
                            }
                        }
                    }
                }
        ...
        private void TxtDOB_Click(object sender, EventArgs e)
        {
            using (MonthCalendar calendar = new MonthCalendar())
            {
                ...
                calendar.DateSelected += (s, args) =>
                {
                    txtDOB.Text = args.Start.ToString("yyyy-MM-dd");
                    calendarForm.Close();
                    txtDOA.Focus();
                };
                calendarForm.Controls.Add(calendar);
        ...
        private void TxtDOA_Click(object sender, EventArgs e)
        {
            using (MonthCalendar calendar = new MonthCalendar())
            {
                Form calendarForm = new Form
                {
                    Text = "Select Date of Appointment",
                    Size = new Size(250, 220),
                    FormBorderStyle = FormBorderStyle.FixedDialog,
                    StartPosition = FormStartPosition.CenterParent,
                    MaximizeBox = false,
                ...
                calendar.MaxSelectionCount = 1;
                calendar.Location = new Point(10, 10);
                if (DateTime.TryParse(txtDOA.Text, out DateTime selectedDate))
                {
                    calendar.SetDate(selectedDate);
                }
                calendar.DateSelected += (s, args) =>
                {
                    txtDOA.Text = args.Start.ToString("yyyy-MM-dd");
                    calendarForm.Close();
                    txtCNIC.Focus();
                };
                calendarForm.Controls.Add(calendar);
        ...
        private void comboBox_txtDeptID_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (comboBox_txtDeptID.SelectedItem != null)
            {
                var selectedDept = (dynamic)comboBox_txtDeptID.SelectedItem;
                txtDeptID.Text = selectedDept.DeptID.ToString();
                txtDepoNo.Text = selectedDept.DeptName.ToString();
            }
        }
        ...
        private void SetFormFieldsEnabled(bool enabled)
        {
            ...
            comboBox_txtDeptID.Enabled = enabled;
            txtDOB.Enabled = enabled;
            txtDOA.Enabled = enabled;
            txtCNIC.Enabled = enabled;
            txtGSalary.Enabled = enabled;
            txtDepoNo.Enabled = false; // Always disabled
            txtDeptID.Enabled = false; // Always disabled
                    ...
                    using (Form gridForm = new Form())
                    {
                        ...
                        dataGridView.CellDoubleClick += (s, args) =>
                        {
                            if (dataGridView.CurrentRow != null)
                            {
                                ...
                                // Set comboBox_txtDeptID to match DeptID
                                foreach (var item in comboBox_txtDeptID.Items)
                                {
                                    if (((dynamic)item).DeptID.ToString() == txtDeptID.Text)
                                    {
                                        comboBox_txtDeptID.SelectedItem = item;
                                        break;
                                    }
                                }
                                SetFormFieldsEnabled(true);
        ...
        {
            // No action needed as txtDeptID is read-only
        }
    }
}